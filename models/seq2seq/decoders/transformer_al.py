...
from models.seq2seq.maskctc.add_mask_token import mask_uniform
from models.seq2seq.maskctc.mask import square_mask
...
class TransformerDecoder(DecoderBase):
    def __init__(self, special_symbols,
        ...
        with open(args.dict, 'r', encoding='UTF-8') as f:
            for x in f:
                self.labels.append(x.split()[0].replace('<space>', ' '))


        if args.unit == 'wp' and int(args.recog_beam_width )>1:
    ...
    def greedy(self, eouts, elens):
        ...
        [ ys_in[0].insert(i, 1) for i in range(len(ys_in[0])-1, 0, -1) if ys_in[0][i] == ys_in[0][i-1]] 
        ys_in = torch.LongTensor(ys_in).to(self.device)
        # return ys_in.cpu().detach().numpy().tolist(), None

        for b in range(eouts.size(0)):
            [ ys_in[b].insert(i, 1) for i in range(len(ys_in[b])-1, 0, -1) if ys_in[b][i] == ys_in[b][i-1]] 
        try:
        ...
        if self.corpus == 'aishell':
            ys_hat = ''.join([self.labels[x] for x in ys_hat])
        elif self.corpus == 'librispeech':
            ys_hat = ''.join([self.labels[x] for x in ys_hat]).replace('▁',' ').strip()
    ...
    def beam_search(self, eouts, elens, params):
        ...
        logits = self.ctc.eouts_output(eouts).detach().cpu().numpy()[0]
        ...
        logits = self.output(self.norm_out(out)).detach().cpu().numpy()[0]
